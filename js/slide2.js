(function($){  // 매개변수(파라미터 Parameter)
    ...
    const obj = {  // 객체 리터럴 방식 권장
        ...
        section1(){
            ...
            // 모든 슬라이드를 position absolute로 해준다.
            // 맨 위 그리고 아래 포게진 슬라이드 앞뒤 순서를 정한다. z-index 3, 2, 1
            // 페이드 아웃 효과는 opactity:0 를 사용한다. 시간은 0.6s 또는 1s
            // 계속 10개의 슬라이드를 반복적으로 회전하여 순서대로 보여진다.
            mainNextSlide(); // 로딩시 실행
            function mainNextSlide(){
                slide.css({zIndex: 1, opacity: 1});
                slide.eq(cnt).css({zIndex:2});
                // cnt=0 이면 마지막슬라이드번호 9
                // 그렇지 않으면 cnt -1 
